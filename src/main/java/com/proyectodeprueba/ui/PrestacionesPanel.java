...
import com.proyectodeprueba.ui.subventanas.AgregarPrestacionPanel;
import com.proyectodeprueba.ui.subventanas.EliminarDialog;
import com.proyectodeprueba.ui.subventanas.EliminarPrestacionPanel;
import com.proyectodeprueba.ui.subventanas.ModificarDialog;
import com.proyectodeprueba.ui.subventanas.ModificarPrestacionPanel;
 ...
 */
public class PrestacionesPanel extends javax.swing.JPanel {
   ...
   private boolean filtrarPorFecha = false;
     ...
     */
    public PrestacionesPanel(MainFrame parent) {
        ...
        cargarPrestacionesEnTabla(dao.obtenerPrestacionesFiltradas(0));
    }
    ...
    private void configurarInterfazPorRol() {
        ...
        if (SessionManager.esAdmin()) {
            ...
            // En el panel de prestaciones, el capturista ve "Prestaciones" (pero no hace nada especial)
            ...
            // No agregar nueva acción - ya está en prestaciones
        } else {
    ...
    private void actualizarTablaConFiltros() {
        ...
        PrestacionDAO dao = new PrestacionDAO();
        cargarPrestacionesEnTabla(dao.obtenerPrestacionesFiltradas(idDeptoSeleccionado, fechaSeleccionada));
    }

    // Este método ahora recibe una lista para ser más flexible
   // Reemplaza tu método actual por este
    private void cargarPrestacionesEnTabla(List<Prestacion> listaPrestaciones) {
        DefaultTableModel model = (DefaultTableModel) JTable.getModel();
        model.setRowCount(0);

        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");

        for (Prestacion p : listaPrestaciones) {
    ...
    private void actualizarTabla() {
        ...
        PrestacionDAO dao = new PrestacionDAO();
        cargarPrestacionesEnTabla(dao.obtenerPrestacionesFiltradas(idDeptoSeleccionado));
    }
    ...
    private void initComponents() {
        ...
        lblUser.setText("Usuario: Capturista");
        ...
        JTable.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "ID Prestacion", "ID Empleado", "Tipo Prestacion", "Monto", "Descripción", "Fecha Agregado"
            }
        ) {
    ...
    private void btnAgregarMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnAgregarMouseClicked
        ...
        AgregarPrestacionPanel agregarPanel = new AgregarPrestacionPanel(agregarDialogo, this::actualizarTabla);
    ...
    private void btnModificarMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnModificarMouseClicked
        ...
        DefaultTableModel model = (DefaultTableModel) JTable.getModel();
        int idPrestacion = (int) model.getValueAt(filaSeleccionada, 0);
        int idEmpleado = (int) model.getValueAt(filaSeleccionada, 1);
        String tipoPrestacion = (String) model.getValueAt(filaSeleccionada, 2);
        java.math.BigDecimal monto = (java.math.BigDecimal) model.getValueAt(filaSeleccionada, 3);
        ...
        // 4. Crear un objeto Prestacion con los datos para enviarlo
        Prestacion prestacionParaModificar = new Prestacion();
        prestacionParaModificar.setIdPrestacion(idPrestacion);
        prestacionParaModificar.setIdEmpleado(idEmpleado);
        prestacionParaModificar.setTipoPrestacion(tipoPrestacion);
        prestacionParaModificar.setMonto(monto);
        prestacionParaModificar.setDescripcion(descripcion);

        // Parseamos la fecha del String de la tabla a un objeto LocalDateTime
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
        prestacionParaModificar.setFecha(java.time.LocalDateTime.parse(fechaHoraStr, formatter));
        ...
        ModificarPrestacionPanel modificarPanel = new ModificarPrestacionPanel(modificarDialog, prestacionParaModificar, this::actualizarTabla);
    ...
    if (filaSeleccionada == -1) {
        JOptionPane.showMessageDialog(this, "Por favor, seleccione una prestacion para eliminar.", "Fila no seleccionada", JOptionPane.WARNING_MESSAGE);
        return; // Si no hay selección, no continúa
    }

    // 3. Captura el ID de la primera columna (índice 0)
    int idPrestacion = (int) JTable.getValueAt(filaSeleccionada, 0);
    ...
    EliminarPrestacionPanel eliminarPanel = new EliminarPrestacionPanel(eliminarDialogo, idPrestacion, this::actualizarTabla);
